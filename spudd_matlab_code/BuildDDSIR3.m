...
function tree_DD = BuildDDSIR3(nliste,Id,pr,pd,pir,pid,nr,nd,flag)
precision_n=1000;
if isempty(nliste)
    % Leaf!
    pR=round((pr+(1-pr)*(1-(1-pir)^nr))*precision_n)/precision_n;
    pD=abs(max(0,round((1-pR)*(pd+(1-pd)*(1-(1-pid)^nd))*precision_n)/precision_n));
    ...
    tree_DD = DDnode.myNew(nliste(1), ...
        [BuildDDSIR3(nliste(2:1:longueur),Id,pr,pd,pir,pid,nr,nd,flag), ...    % Available
        BuildDDSIR3(nliste(2:1:longueur),Id,pr,pd,pir,pid,nr+1,nd,flag), ...     % Reserved
        BuildDDSIR3(nliste(2:1:longueur),Id,pr,pd,pir,pid,nr,nd+1,flag)]);       % Dev
end
