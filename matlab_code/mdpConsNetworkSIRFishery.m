function ddMDP=mdpConsNetworkSIRFishery(G,pr,pd,pir,pid,cost,BR,Model)
...
parentWeights = cell(n,1); % list weights of interactions
for i = 1:n
    count=1;
    for j=1:n
        ...
        if j~=i % && G(i,j)~=0
            ...
            parentWeights{j} = [parentWeights{j},G(i,j)];
            count=count+1;
        end
...
elseif Model==3 %reserved influences reservation and developed influences development
    ddMDP=fillddMDPNetSIR3Fishery(pr,pd,pir,pid,cost,BR,parentParcels,parentWeights);
else
